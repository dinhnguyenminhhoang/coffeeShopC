...
import { getDetailRating, getListRatting } from "../../service/rating";

const FeedbackRatingForm = ({ handleClose, onSave, id, isVisible }) => {
    ...
    useEffect(() => {
        if (id) {
            getDetailRating(id).then((response) => {
                if (response.data?.Success) {
                    console.log(response.data);
                    form.setFieldValue(
                        "feedback",
                        response.data?.ResultData?.Feedback?.FeedbackContent
                    );
                }
            });
